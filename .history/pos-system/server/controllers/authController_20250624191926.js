...
// const Otp = require('../models/Otp'); // No longer needed for email OTP
...
const registerUser = async (req, res) => {
    try {
        const { name, employeeId, phone, password, role, email } = req.body; // Added email
        if (!name || !employeeId || !phone || !password || !role || !email) {
            return res.status(400).json({ message: 'Please provide all required fields.' });
        }
        const existingUser = await User.findOne({ $or: [{ employeeId }, { email }] });
        if (existingUser) {
            return res.status(400).json({ message: 'A user with this Employee ID or Email already exists.' });
        }
