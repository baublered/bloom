...
import './RetailPayment.css'; // Both payment pages can share the same CSS for a consistent look
...
function EventsPayment() {
  ...
  useEffect(() => {
    ...
    if (!isNaN(dp) && dp >= 0) { // Ensure down payment is a valid number
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // --- UPDATED: Navigate to receipt page after saving ---
        setTimeout(() => navigate('/receipt', { 
            state: { 
                ...
                discountAmount, 
                paymentMethod,
                eventDetails, // Pass event details for the receipt
                amountPaid: parseFloat(downPayment) || 0, // The amount paid is the down payment
                change: 0, // No change calculated for event down payments
             }
        }), 2000);
      } else {
