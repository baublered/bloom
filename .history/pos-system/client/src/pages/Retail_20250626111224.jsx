...
const Retail = () => {
    ...
    setCart(prevCart => {
      if (isChecked) {
        if (!prevCart.find(item => item._id === product._id) && product.quantity > 0) {
          // Add a flag to distinguish regular flowers from add-ons
  ...
  const handleAddOnCheck = (product, isChecked) => {
    setCart(prevCart => {
        if (isChecked) {
            // Mark this item as an add-on so it's not removed
  ...
  // --- UPDATED: This function now correctly handles the cart ---
  const addBouquetToCart = () => {
      ...
      setCart(prevCart => {
          // 1. Keep only the items that are specifically add-ons
          const addOnsOnly = prevCart.filter(item => item.isAddOn);
          // 2. Add the new bouquet to the cart, preserving only the add-ons
          return [...addOnsOnly, newBouquet];
      });

      // Reset bouquet builder state
      setBouquetFlowers({});
