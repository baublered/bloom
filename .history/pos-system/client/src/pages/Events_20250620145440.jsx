...
const Events = () => {
  ...
  const handleFormSubmit = (e) => {
    ...
    navigate('/product-selection', { state: { eventDetails: newEvent } });
  };
  ...
  const handleRemoveEvent = (eventIdToRemove) => {
    ...
    if (window.confirm("Are you sure you want to remove this event?")) {
        const updatedEvents = events.filter(event => event.id !== eventIdToRemove);
        setEvents(updatedEvents);
        localStorage.setItem('events', JSON.stringify(updatedEvents));
        // Close the modal after removing the event
        setIsEventListModalOpen(false);
    }
  };
              ...
              {eventsForSelectedDate.map(event => (
                <div key={event.id} className="event-list-item">
                  ...
                  <div className="event-list-actions">
                    ...
                    {/* --- NEW: Remove button --- */}
                    <button className="remove-event-button" onClick={() => handleRemoveEvent(event.id)}>
                      Remove
                    </button>
                  </div>
                </div>
