...
const VerifyOTP = () => {
  ...
  const email = location.state?.email; // <-- Use email
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/auth/verify-otp', {
        ...
        body: JSON.stringify({ email, otp }), // <-- Use email
      });
  ...
  const handleContinue = (e) => {
    e.preventDefault();
    if (isVerified) {
      navigate('/reset-password', { state: { email } }); // <-- Use email
    } else {
  ...
  return (
    <div className="verify-otp-page">
      ...
      <div className="form-card">
        ...
        <p className="instructions">An OTP has been sent to {email}.</p> {/* <-- Show email */}
