  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setTimeout(() => {
          // Navigate to a new Receipt page after successful payment
          navigate('/receipt', { 
            state: { 
                orderSummary, 
                grandTotal, 
      ...
      console.error('Payment confirmation failed. Full error object:', error);
      
      // --- ENHANCED ERROR HANDLING ---
      // This will now show the specific error message from your backend.
      if (error.response) {
        // The server responded with a status code outside the 2xx range
        console.error('Backend response data:', error.response.data); // Log the exact response
        const serverMessage = error.response.data.message || error.response.data.error;
        setStatusMessage(`❌ Error: ${serverMessage || 'The server returned an unhandled error.'}`);
