...
function RetailTransaction() {
  ...
  const [addOnList, setAddOnList] = useState([]); // State for add-ons
  ...
  // Fetch products (flowers and add-ons) from inventory
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await axios.get('http://localhost:5000/api/products');
        console.log("Fetched products:", res.data);  // Log the response data to see the structure

        // Separate flowers and add-ons based on category
        const flowers = res.data.filter((product) => product.category === 'flowers');
        const addOns = res.data.filter((product) => product.category === 'add-ons');
        
        setFlowerList(flowers);  // Set flowers list
        setAddOnList(addOns);    // Set add-ons list
      } catch (err) {
        console.error(err);
  ...
  // Flower selection and quantity update
  ...
  const handleProceed = () => {
    // Prepare selected flower data to send to the next page
          ...
          <div className="flower-list">
            ...
            <div className="flower-container">
              {flowerList.length > 0 ? (
                ...
                <p>No flowers available</p>  // Fallback message for flowers
              )}
            </div>
          </div>
          ...
          <div className="add-ons">
            ...
            <div className="add-on-container">
              {addOnList.length > 0 ? (
                ...
                <p>No add-ons available</p>  // Fallback message for add-ons
              )}
            </div>
          </div>
