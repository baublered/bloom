...
function RetailTransaction() {
  ...
  const updateQuantity = (flower, amount, stock) => {
    setSelectedFlowers((prev) => {
      ...
      const newQty = Math.max(Math.min(currentQty + amount, stock), 1); // Ensure quantity doesn't exceed stock
      if (newQty > stock) {
                ...
                <div className="flower-info">
                  <div className="flower-title">
                    ...
                    <div className="stock-info">
                      <span>{stock} Available</span> {/* Display available stock */}
                    </div>
                    <div className="price-info">
                      <span>₱{price}</span> {/* Replace $ with ₱ */}
                    </div>
                  </div>
                  <input
                    type="checkbox"
