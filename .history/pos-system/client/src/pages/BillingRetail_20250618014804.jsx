...
function BillingRetail() {
  ...
  // This robust function gets cart data safely, preventing crashes.
  const getCartData = () => {
  ...
  const applyDiscount = async () => {
    ...
    try {
      ...
      // --- FIX START ---
      // Check for the correct property name from your backend: 'discount_amount'
      if (data.valid && typeof data.discount_amount === 'number') {
        if (data.discountType === 'percentage') {
          // Assuming the percentage is also in 'discount_amount'
          const discountValue = (subtotal * data.discount_amount) / 100;
          setDiscountAmount(discountValue);
        } else {
          // For fixed amounts, use 'discount_amount' directly
