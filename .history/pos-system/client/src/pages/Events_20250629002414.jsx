                ...
                {events.map(event => (
                    <div key={event._id} className="popover-event-item">
                        <div className="event-title">
                            <strong>{event.eventType} - {event.customerName}</strong>
                        </div>
                        {event.status === 'Pending' && (
                            <>
                                <div className="event-balance" style={{ color: '#ffb300', fontWeight: 'bold', marginBottom: '8px' }}>
                                    BALANCE: {event.remainingBalance?.toLocaleString() || 0}
                                </div>
                                <div><strong>Customer Name:</strong> {event.customerName}</div>
                                <div><strong>Address:</strong> {event.address}</div>
                                <div><strong>Phone Number:</strong> {event.phone}</div>
                                <div><strong>Event:</strong> {event.eventType}</div>
                                <div><strong>Amount of Down Payment:</strong> {event.downPayment?.toLocaleString() || 0}</div>
                                <button className="popover-btn pay" onClick={() => onView(event)}>Pay</button>
                            </>
                        )}
                        {event.status === 'Fully Paid' && (
                            <>
                                <div style={{ color: '#1abc1a', fontWeight: 'bold', marginBottom: '8px' }}>
                                    FULLY PAID
                                </div>
                        ...
                        {event.status === 'Cancelled' && (
                            <>
                                <div style={{ color: '#e53935', fontWeight: 'bold', marginBottom: '8px' }}>
                                    CANCELLED
                                </div>
                            </>
                        )}
