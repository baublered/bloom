  ...
  const handleConfirmPayment = async () => {
    ...
    const transactionData = {
        ...
        items: orderSummary.map(item => {
            if (item.isBouquet) {
                // For bouquets, send the component flowers for stock validation
                return {
                    _id: item._id, // Keep the bouquet ID (timestamp-based)
                ...
                // For individual items, send normal product data
                return {
                    _id: item._id,
                    productName: item.productName,
                    quantity: item.quantity,
